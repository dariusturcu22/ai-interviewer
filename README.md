...
**Live app:** [ai-interviewer-indol-five.vercel.app](https://ai-interviewer-indol-five.vercel.app)
**Backend API:** [ai-interviewer-2gzt.onrender.com](https://ai-interviewer-2gzt.onrender.com)
...
Turbopack, since Turbopack's file watcher doesn't pick up changes through a Docker bind mount on
...
For the fastest dev-server experience, running the frontend natively instead of in Docker.
...
Still requires a local Postgres reachable at the `DATABASE_URL` in `backend/.env`; the easiest
way is `docker compose up postgres`, just without the backend container.
...
but not appropriate for production at scale.
...
backend/            FastAPI app, tests
  app/core/          database engine/session, rate limiting
  app/interviews/    models, schemas, routes, the Anthropic tool-use client, keyword extraction
  app/main.py        app entrypoint: CORS, exception handlers, router wiring
